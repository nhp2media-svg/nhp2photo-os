...
const TASK_DEFS={
  ...
  face_replace:{risk:"high", optionsHTML:`
...
function riskScore(){
  ...
  let score=10;
  let notes=[];
  if(wf==="archival"){score+=6;notes.push("Archival: higher sensitivity to historical truth.");}
  ...
  if(t==="pose") notes.push("Pose/angle synthesizes geometry; keep degrees low.");
...
function updateRisk(){
  ...
  badge.textContent = score>=60 ? "High" : score>=35 ? "Medium" : "Low";
  badge.style.borderColor = score>=60 ? "rgba(255,90,122,.45)" : score>=35 ? "rgba(255,204,102,.45)" : "rgba(36,209,139,.45)";
  badge.style.background = score>=60 ? "rgba(255,90,122,.12)" : score>=35 ? "rgba(255,204,102,.12)" : "rgba(36,209,139,.12)";
  el("riskNotes").textContent = notes.join(" ");
}
...
function workflowBlock(){
  ...
  return wf==="archival"
    ? ["ARCHIVAL WORKFLOW: Preserve historical truth and documentary accuracy.",
       ...
       "Do not modernize the photo (no “new camera” look)."].join("\n")
    : ["COMMERCIAL/EDITORIAL WORKFLOW: Publication-ready polish without identity drift.",
       "Allow gentle cleanup and clarity, but forbid cosmetic reshaping."].join("\n");
}
...
function taskDirectives(){
  ...
  if(t==="enhance"){
    ...
    lines.push(`- Enhance full image realism at ${String(strength).toUpperCase()} strength.`);
    ...
    lines.push(`- White balance: ${String(wb).replace('_',' ')}.`);
    lines.push("- Preserve skin tone and original ambient cast unless correction is explicitly required.");
  } else if(t==="texture"){
...
function copyOut(){ navigator.clipboard.writeText(el("out").value||""); }
...
function lintPrompt(){
  ...
  out.innerHTML="";
  const rules=[
    {k:["average","averaging","blend identities","face merge"], lvl:"bad", msg:"Identity blending/averaging increases drift. Prefer identity lock + strict match."},
    {k:["de-age","younger","beautify","perfect skin","symmetry"], lvl:"warn", msg:"Beautification/de-aging cues often rewrite faces. Forbid beautification; preserve age cues."},
    {k:["hdr","cinematic","teal orange","bloom","glow"], lvl:"warn", msg:"Cinematic/HDR language pushes stylization. Prefer neutral photographic corrections."},
    ...
    {k:["8k","upscale","4x"], lvl:"good", msg:"Upscaling is fine if you forbid invented micro-detail and keep grain/texture natural."},
  ];
  let hits=0;
  for(const r of rules){
    if(r.k.some(w=>txt.includes(w))){
      ...
      d.style.borderColor = r.lvl==="bad" ? "rgba(255,90,122,.35)" : r.lvl==="warn" ? "rgba(255,204,102,.35)" : "rgba(36,209,139,.35)";
      d.innerHTML=`<div style="font-weight:900">${r.lvl.toUpperCase()}</div><div class="muted" style="margin-top:6px">${r.msg}</div>`;
      out.appendChild(d);
    }
  }
  if(!hits){
    ...
    d.style.boxShadow="none";
    d.style.marginTop="10px";
    d.style.borderColor="rgba(36,209,139,.35)";
    d.innerHTML=`<div style="font-weight:900">GOOD</div><div class="muted" style="margin-top:6px">No common drift triggers detected.</div>`;
    out.appendChild(d);
  }
}
...
function libRender(list){
  ...
  host.innerHTML="";
  if(!list.length){ host.innerHTML=`<div class="muted">No matches.</div>`; return; }
  list.forEach(item=>{
    ...
    card.innerHTML=`
      ...
      <textarea spellcheck="false" style="min-height:150px;margin-top:8px">${item.text}</textarea>
    `;
...
function libApply(){
  ...
  libRender(LIB.filter(p=>(m==="any"||p.mode===m)&&(t==="any"||p.task===t)));
}
...
function init(){
  ...
  ["wf","platform","lock_identity","keep_grain","no_hdr","no_beauty","no_invent"].forEach(id=>{
    el(id).addEventListener("change", updateRisk);
  });
